...
.brafilter a {
    ...
    box-shadow: inset 0 0 0 1px #DBDBDB, 0 0 0 1px #fff, 2px 2px 4px rgba(0,0,0,0.3);
...
.brafilter a:hover { color: #01487e; background: -webkit-gradient(linear, center top, center bottom, from(#DBDBDB), to(#fff)); background-image: linear-gradient(#DBDBDB, #fff); box-shadow: inset 0 0 0 1px #01487e, 0 0 0 1px #01487e, 2px 2px 4px rgba(0,0,0,0.3); }
...
if(count($arApps)>0){
	...
	//if($arType['TYP_PCON_END']>199500)
	{
		...
		<td><a class="dblock" target="_blank" href="<?=$arType['URL']?>"><b><?=$arType['MOD_CDS_TEXT']?></b> <?=$arType['TYP_CDS_TEXT']?></a></td>
...
$(document).ready(function () {
	...
	LetsDiv.click(function (){
		...
		TabRows.hide().delay(50).each(function(i){
			var AText = $(this).children('.fbra').eq(0).text();
			if(RegExp('^' + FstLet).test(AText)) {
				$(this).fadeIn(400);
			}
		});
		jQuery('div#AppsListModels').height(jQuery(window).height()*0.95-jQuery('.brafilter').height()-100);
