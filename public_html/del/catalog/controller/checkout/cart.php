...
class ControllerCheckoutCart extends Controller {
    	...
    	if ($this->cart->hasProducts() || !empty($this->session->data['vouchers'])) {
			...
			$products = $this->cart->getProducts();
      		...
      		foreach ($products as $product) {
				...
				if($product['tecdoc']=="Y"){
					$arPFields['tecdoc'] = "Y";
					$arPFields['thumb'] = $product['image'];
					$arPFields['href'] = $product['product_url'];
				}
				//////////////////////////////////////////
				
				$this->data['products'][] = $arPFields;
	...
	public function add() {
		...
		if ($product_info) {			
			...
			if (!$json) {
				...
				$json['success'] = sprintf($this->language->get('text_success'), $this->url->link('product/product', 'product_id=' . $this->request->post['product_id']), $product_info['name'], HTTP_SERVER . 'index.php?route=checkout/simplecheckout');
	...
	public function quote() {
		...
		if (!$json) {		
			$this->tax->setShippingAddress(220, $this->request->post['zone_id']);
		
			// Default Shipping Address
			$this->session->data['shipping_country_id'] = 220;
    ...
    public function update()
    {
        ...
        foreach ($this->cart->getProducts() as $result) {
			...
			if($result['tecdoc']=="Y"){
				$arPFields['tecdoc'] = "Y";
				$arPFields['thumb'] = $result['image'];
				$arPFields['href'] = $result['product_url'];
			}
			//////////////////////////////////////////
