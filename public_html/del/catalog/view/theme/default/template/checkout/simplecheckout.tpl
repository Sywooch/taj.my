...
<?php if (!$ajax) { ?>
	...
	<div class="order-one-click" style="display:none;">
		<h4>Заказать без оформления</h4>
		<p>Можно оформить заказ без заполнения форм. Для этого достаточно указать свой телефон и консультант свяжется с Вами. Товары из корзины добавятся в заказ автоматически.</p>
		<div class="clearfix">
			<input type="text" name="one-phone" class="one-phone" placeholder="Введите номер телефона" />
			<input type="submit" name="one-submit" class="one-submit" value="Заказать в один клик" />
		</div>
		<script>
		$(document).ready(function(){
		$('[name="one-phone"]').mask('+38 (999) 999-99-99');
		})
		</script>
	</div>
