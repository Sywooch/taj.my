...
class ControllerProductCategory extends Controller {  
	public function index() { 
			...
			foreach ($parts as $path_id) {
				...
				if ($category_info) {
	       			$this->data['breadcrumbs'][] = array(
						...
						'href'      => $this->url->link('product/category', 'path=' . $path . $url . $filter_url),
		...
		if ($category_info) {
			...
			$this->data['breadcrumbs'][] = array(
				'text'      => $category_info['name'],
				'href'      => $this->url->link('product/category', 'path=' . $this->request->get['path'] . $filter_url),
				'separator' => $this->language->get('text_separator')
			);
			...
			$parent_id = $this->model_catalog_category->getCategoryParent($category_id); 
			...
			foreach ($results as $result) {
				...
				if($parent_id == 0) {
					...
					foreach($ch_r as $ch_i) {
						...
						$ch[] = array(
							'href' => $this->url->link('product/category', 'path=' . $this->request->get['path'] . '_' . $result['category_id'] . '_' . $ch_i['category_id'] . $url . $filter_url),
							'name' => $result['name'] . ($this->config->get('config_product_count') ? ' <span>(' . $product_total_c . ')</span>' : '')
						);
					}
				}
				$this->data['categories'][] = array(
					'name'  => $result['name'] . ($this->config->get('config_product_count') ? ' <span>(' . $product_total . ')</span>' : ''),
					'href'  => $this->url->link('product/category', 'path=' . $this->request->get['path'] . '_' . $result['category_id'] . $url . $filter_url),
