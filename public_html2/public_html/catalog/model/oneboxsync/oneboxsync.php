...
//----------------------------------------------------UPDATE CATEGORY---------------------------------------------------
    ...
    public function newCategory ($id, $parentid, $name, $name_lang, $ext_id) {
        $top = 0;
        if($parentid == 0){
            $top = 1;
        }
        $this->db->query("INSERT INTO `" . DB_PREFIX . "category` (`category_id`, `parent_id`, `top`, `status`, `date_added`, `date_modified`, `ext_id`) values ('" . $id . "', '" . $parentid . "', '" . $top . "', '1', NOW(), NOW(), '" . $ext_id . "')" );
    ...
    public function updateCategory ($id, $parentid, $name, $name_lang, $ext_id) {
        $top = 0;
        if($parentid == 0){
            $top = 1;
        }
        $this->db->query("REPLACE INTO `" . DB_PREFIX . "category_to_store` (`category_id`, `store_id`) values ('" . $id . "', '0')" );
        $this->db->query("UPDATE `" . DB_PREFIX . "category` SET `parent_id` = '" . $parentid . "', `top` = '" . $top . "', `date_modified` = NOW(), `ext_id` =  '" . $ext_id . "' WHERE `category_id` = " . $id  );
...
//----------------------------------------------UPDATE PRODUCT----------------------------------------------------------
    ...
    public function newImageValue ($id, $image)
    {
        ...
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_image WHERE product_id = '" . $id . "' && image = '" . $image . "'");
        $this->db->query("DELETE FROM " . DB_PREFIX . "product_image WHERE product_id = '" . $id . "' && image = 'image/" . $image . "'");
    ...
    public function newProduct ($id, $articul, $quantity, $image, $manufacturer, $price, $avail, $name, $name_lang, $description){
        $this->db->query("INSERT INTO " . DB_PREFIX . "product (product_id, model, quantity, image, manufacturer_id, price, status, date_added, date_modified) values ('" . $id . "', '" . $articul . "', '" . $quantity . "', '" . $image ."', '" . $manufacturer . "', '" . $price . "', '" . $avail . "', NOW(), NOW())" );
        $this->db->query("INSERT INTO " . DB_PREFIX . "product_to_store (product_id, store_id) values ('" . $id . "', '0')" );
        foreach ($name_lang as $value){
            if ($value['name'] == '') {
                $value['name'] = $name;
            }
            $this->db->query("INSERT INTO `" . DB_PREFIX . "product_description` (`product_id`, `language_id`, `name`, `description`) values ('" . $id . "', '" . $value['lang_id'] . "', '" . $value['name'] . "', '" . $this->db->escape($description) . "')" );

        }
        return $id;
    }
    
    public function updateProduct ($id, $articul, $quantity, $image, $manufacturer, $price, $avail, $name, $name_lang, $description)	{
        $this->db->query("UPDATE " . DB_PREFIX . "product SET model = '" . $articul . "', quantity = '" . $quantity . "', image = '" . $image ."', manufacturer_id = '" . $manufacturer . "', price = '" . $price . "', status = '" . $avail . "', date_modified = NOW() WHERE  product_id = '" . $id . "'" );
        $this->db->query("REPLACE INTO " . DB_PREFIX . "product_to_store (product_id, store_id) values ('" . $id . "', '0')" );

        foreach ($name_lang as $value){
            $name_base = $this->db->query("SELECT  `" . DB_PREFIX . "product_description`.`name` as name FROM `" . DB_PREFIX . "product_description` WHERE `" . DB_PREFIX . "product_description`.`product_id` = '" . $id . "' && `" . DB_PREFIX . "product_description`.`language_id` = '" . $value['lang_id'] . "'");

            if ($value['name'] == '') {

                if ($name_base->row['name'] == '') {
                    $value['name'] = $name;
                } else {
                    $value['name'] = $name_base->row['name'];
                }
            }
            if ($name_base->num_rows == 0){
...
//------------------------------------------------------ ORDER----------------------------------------------------------
    ...
    public function updateStatusOrder ($status_id, $order_id)
	{
...
//----------------------------------------------------UPDATE FILTRE NAME------------------------------------------------
    ...
    public function updateStandartFiltreName($id, $name, $name_lang){
        ...
        if ($is_filter->num_rows == 0){
            $group_id = $this->db->query ("SELECT MAX(  `filter_group_id` ) as maxgroup FROM  `" . DB_PREFIX . "filter_group_description`");

            $group_id = $group_id->row['maxgroup'] + 1;
            ...
            foreach ($name_lang as $value) {
                ...
                $this->db->query("INSERT INTO `" . DB_PREFIX . "filter_description` (`filter_id`, `language_id`, `filter_group_id`,  `name`) values ('" . $id . "', '" . $value['lang_id'] . "', '" . $group_id . "','" . $value['name'] . "')");
            }
            $this->db->query("INSERT INTO `" . DB_PREFIX . "filter_group` (`filter_group_id`, `sort_order`) values ('" . $group_id . "', '0')");
            $this->db->query("INSERT INTO `" . DB_PREFIX . "filter` (`filter_id`, `filter_group_id`, `sort_order`) values ('" . $id . "', '" . $group_id . "', '0')");

        } else {
            $group_id = $this->db->query ("SELECT  `filter_group_id` FROM  `" . DB_PREFIX . "filter_description` WHERE `filter_id`='" . $id . "'");
            $group_id = $group_id->rows[0]['filter_group_id'];
            ...
            foreach ($name_lang as $value) {
                ...
                $this->db->query("REPLACE INTO `" . DB_PREFIX . "filter_description` (`filter_id`, `language_id`, `filter_group_id`,  `name`) values ('" . $id . "', '" . $value['lang_id'] . "', '" . $group_id . "','" . $value['name'] . "')");
            }
            $this->db->query("REPLACE INTO `" . DB_PREFIX . "filter_group` (`filter_group_id`, `sort_order`) values ('" . $group_id . "', '0')");
            $this->db->query("REPLACE INTO `" . DB_PREFIX . "filter` (`filter_id`, `filter_group_id`, `sort_order`) values ('" . $id . "', '" . $group_id . "', '0')");
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    /*
       public function updateFiltreName ($id, $lang_ru, $name_ru, $lang_ua, $name_ua)
       {
           $this->db->query("REPLACE INTO " . DB_PREFIX . "filter_description (option_id, language_id, name, filter_group_id) values ('" . $id . "', '" . $lang_ru . "', '" . $name_ru . "', '1' )");
           $this->db->query("REPLACE INTO " . DB_PREFIX . "filter_description (option_id, language_id, name, filter_group_id) values ('" . $id . "', '" . $lang_ua . "', '" . $name_ua . "', '1' )");
           $this->db->query("REPLACE INTO " . DB_PREFIX . "filter (filter_id, filter_group_id, sort_order) values ('" . $id . "','1', '1')");
           return 'updated';
       }


       public  function  updateFiltreProduct($filterid, $productid, $filtervalue){

           $this->db->query("REPLACE INTO " . DB_PREFIX . "product_filter (product_id, filter_id) values ('" . $productid . "', '" . $filterid ."')" );
   //        $this->db->query("REPLACE INTO " . DB_PREFIX . "ocfilter_option_value_description (value_id, option_id, language_id, name) values ('" . $valueId->row['value_id'] . "', '" . $filterid . "', '1', '" . $filtervalue . "')" );
           return 'updated';
       }
   */
...
//		SELECT  oc_product.product_id, oc_product.image as image_osn, oc_product.status, oc_product.price, oc_product.model, oc_product_description.name, oc_product_description.description, oc_product_to_category.category_id, oc_product_image.image, oc_manufacturer.name as manufacturername, code FROM oc_product, oc_product_description, oc_product_to_category, oc_product_image, oc_language, oc_manufacturer WHERE oc_product.product_id = oc_product_description.product_id && oc_product_to_category.product_id = oc_product.product_id && oc_product_image.product_id = oc_product.product_id && oc_product_description.language_id = oc_language.language_id && oc_manufacturer.manufacturer_id = oc_product.manufacturer_id ORDER BY oc_product.product_id
    ...
    public function getFiltrId ($filtrId, $productId)
    {
        $query = $this->db->query("SELECT option_id FROM " . DB_PREFIX . "ocfilter_option_value_to_product WHERE product_id = '" . $productId . "' && option_id = '" . $filtrId . "'");

        return $query;
    }

    public  function  newFiltreProduct($filterid, $productid, $filtervalue){
      ...
      $cat1 = $this->db->query("SELECT category_id FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . $productid . "'" );
        ...
        $this->db->query("REPLACE INTO " . DB_PREFIX . "ocfilter_option_value (option_id, keyword) values ('" . $filterid . "','" . $filtervalue ."')" );
        $filtervalueId = $this->db->query("SELECT  value_id FROM " . DB_PREFIX . "ocfilter_option_value WHERE keyword = '" . $filtervalue . "'" );
        foreach ($filtervalueId->rows as $result) {
            $valueId = $result;
        }
        $this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_option_value_to_product (product_id, value_id, option_id) values ('" . $productid . "', '" . $valueId['value_id'] . "','" . $filterid ."')" );
        $this->db->query("INSERT INTO " . DB_PREFIX . "ocfilter_option_value_description (value_id, option_id, language_id, name) values ('" . $valueId['value_id'] . "', '" . $filterid . "', '1', '" . $filtervalue . "')" );
        return 'new';
    }


    
    public function isFiltreProductValue ($productId){
        $filterProductValue = array();
        $query = $this->db->query("SELECT " . DB_PREFIX . "ocfilter_option_value_to_product.product_id, " . DB_PREFIX . "ocfilter_option_value_to_product.option_id, " . DB_PREFIX . "ocfilter_option_value_description.name FROM " . DB_PREFIX . "ocfilter_option_value_to_product, " . DB_PREFIX . "ocfilter_option_value_description WHERE " . DB_PREFIX . "ocfilter_option_value_to_product.product_id = '" . $productId . "' && " . DB_PREFIX . "ocfilter_option_value_to_product.value_id = " . DB_PREFIX . "ocfilter_option_value_description.value_id");

        foreach ($query->rows as $result) {
            $filterProductValue[] = $result;
        }

        return $filterProductValue;
    }
}
?>
