...
<div class="simplecheckout-customer-same-address hidden">
...
<?php if ($simple_debug) print_r($comment); ?><script>	if(jQuery('input#checkout_customer_main_city').val()==3)	jQuery('input#checkout_customer_main_city').val('');	jQuery('select#checkout_customer_main_zone_id').change(function() {		jQuery.ajax({			url: '/np_city.php?reg='+jQuery(this).val(),			type: 'get',			success: function(data) {				jQuery('select#checkout_customer_main_city').html(data);			}			});	});	jQuery('select#checkout_customer_main_city').change(function() {		jQuery.ajax({			url: '/np.php?city='+jQuery(this).val(),			type: 'get',			success: function(data) {				jQuery('select#checkout_customer_custom_np').html(data);			}			});	});</script>