...
if($arSup["MULTY_AV"]!='') {			
	$mult_av=explode(',',$arSup["MULTY_AV"]);
}
?>
			...
			if($arCFile = file($arSup['FILE_PATH'])){
				...
				if($CSVcount>$arSup['START_FROM']){
					...
					foreach($arCFile as $Line=>$strLine){
						...
						if($arSup['ARTBRA_SEP']!='' AND $arFields['ARTICLE_BRAND']!=''){
							...
							if(count($arAB)>1){
								if($arSup['ARTBRA_SIDE']==1){
									if(iconv_strlen($arFields['ARTICLE'])<1)
										$arFields['ARTICLE']=$arAB[0]; 
									if(iconv_strlen($arFields['BRAND'])<1)
										$arFields['BRAND']=$arAB[1];
								}elseif($arSup['ARTBRA_SIDE']==2){
									if(iconv_strlen($arFields['ARTICLE'])<1)
										$arFields['ARTICLE']=$arAB[1]; 
									if(iconv_strlen($arFields['BRAND'])<1)
						...
						if($mult_av!='') {
							$arFields["AVAILABLE"]=0;
							foreach($mult_av as $av_n)
							$arFields["AVAILABLE"]+=TDMOnlyNumbers($arCSVrow[$av_n-1]);
						}
						...
						if($arSup['CONSIDER_HOT']!=1 OR $arOps['HOT']!=1){ //Если цена содержит опцию "Распродажа" - то наценку (скидку) не применять
							if($arSup['PRICE_EXTRA']!=0){
								...
								$PRICE_ORIGINAL=$arFields["PRICE"];
								$arFields["PRICE"]=($arFields["PRICE"]+(($arFields["PRICE"]/100)*$arSup['PRICE_EXTRA']));
							}
						}
						
						if(($arSup['CURR']>0) && ($arFields['CURRENCY']=="EUR"||$arFields['CURRENCY']=="USD")) 
						{
							$arFields['CURRENCY']="UAH";
							$arFields["PRICE"]=$arFields["PRICE"]*$arSup['CURR']; 
						}
						
						//if($_POST["CURR_CURSE"]>0) $arFields["PRICE"]=$arFields["PRICE"]*$_POST["CURR_CURSE"];
						...
						// CONFIG BKEY
						$bkey_lower=strtolower($arFields["BKEY"]);
						switch($bkey_lower) {
							case 'seinsa':
								$arFields["BKEY"]=						'Autofren seinsa'; 
								break;
							case 'ate-teves':
							case 'ate- teves':
								$arFields["BKEY"]=						'ATE';
							...
							case 'aisin1':
								...
								break;
							case 'autofren (sein)':
							case 'seinsa autofren':
								$arFields["BKEY"]=						'Autofren seinsa';
								break;
							case preg_match('/bosch/', $bkey_lower) ? true : false:
							case '_bosch':
						/*	case 'boschakumulatory': 
							case 'boschfiltry': 
							case 'bosch diesel':
							case 'boschdiesel':
							case 'bosch paski':
							case 'boschpaski':
							case 'bosch ukrad hamulcowy':
							case 'bosch wtrysk':
							case 'bosch wycieraczki':
							case 'bosch mwiece/przewody':
							case 'bosch-elektronarzedzia':
						*/
								$arFields["BKEY"]=						'BOSCH'; 
								break;
							case 'champion - запчастини':
								$arFields["BKEY"]=						'Champion';
								break;
							case 'peugeot/citroen':
							case 'peugeot-citroen':
								$arFields["BKEY"]=						'Citroen';
								break;
							case 'conti':
							case 'сotnitech':
							case 'conti miechy':
								$arFields["BKEY"]=						'CONTITECH';
								break;
							case 'complex automotive bearings sp.z.o.o.':
								$arFields["BKEY"]=						'Complex';
								break;
							case 'cargo':
							case 'hc-cargo':
								$arFields["BKEY"]=						'HC Cargo';
								break;
							case 'hepu - ipd':
								$arFields["BKEY"]=						'Hepu';
								break;
							case preg_match('/hengst/', $bkey_lower) ? true : false:
								$arFields["BKEY"]=						'Hengst';
								break;
							case 'oe daf':
								$arFields["BKEY"]=						'DAF';
								break;
							case preg_match('/delphi/', $bkey_lower) ? true : false:
						/*	case 'delphi diesel':
							case 'delphi wtrysk':
						*/
								$arFields["BKEY"]=						'Delphi';
								break;
							case preg_match('/denso/', $bkey_lower) ? true : false:
						/*	case 'denso diesel':
							case 'delphi wtrysk':
							case 'delphi klima':
						*/
							...
							case 'facet s.r.l.':
								$arFields["BKEY"]=						'Facet';
								break;
							case preg_match('/fag/', $bkey_lower) ? true : false:
						/*	case 'fag hydraulika':
							case 'fag prezemysr':
						*/
								$arFields["BKEY"]=						'FAG';
								break;
							case 'febi bilstein':
							case 'febi - асортимент':
								$arFields["BKEY"]=						'Febi';
								break;
							case 'oe fiat':
								$arFields["BKEY"]=						'Fiat';
								break;
							case 'fischer automotive one':
								$arFields["BKEY"]=						'Fischer';
								break;
							case 'oe ford':
								$arFields["BKEY"]=						'Ford';
								break;
							case 'kyb':
								$arFields["BKEY"]=						'Kayaba';
								break;
							case 'cargo': 
								$arFields["BKEY"]=						'HC-Cargo'; 
								break;
							...
							case 'kayaba': 
								...
								break;
							case 'ks':
								$arFields["BKEY"]=						'Kolbenschmidt';
								break;
							case 'lemfoerder':
							case 'lmi':
								$arFields["BKEY"]=						'Lemforder';
								break;
							case preg_match('/luk/', $bkey_lower) ? true : false:
								$arFields["BKEY"]=						'LUK';
								break;
							case 'mahle': 
								$arFields["BKEY"]=						'MahleOriginal'; 
								break;
							case preg_match('/mahle /', $bkey_lower) ? true : false:
								$arFields["BKEY"]=						'Mahle'; 
								break;
							case 'mando corporation, seoul': 
								$arFields["BKEY"]=						'Mando'; 
								break;
							case preg_match('/mann/', $bkey_lower) ? true : false:
						/*	case 'mann filter': 
						*/
								$arFields["BKEY"]=						'Mann'; 
								break;
							...
							case 'oe mb': 
							case 'mercedes-benz': 
								$arFields["BKEY"]=						'Mercedes'; 
								break;
							case 'mahle': 
								$arFields["BKEY"]=						'MahleOriginal'; 
								break;
							case 'ssangyong': 
								$arFields["BKEY"]=						'SSANG YONG'; 
								break;
							case 'ntn snr':
							case 'ntn-snr':
								$arFields["BKEY"]=						'SNR'; 
							...
							case 'gkn-spidan': 
							case 'gkn': 
								$arFields["BKEY"]=						'Spidan'; 
								break;
							case 'hans pries': 
							case 'hanspries': 
							case 'hans_pries': 
								$arFields["BKEY"]=						'Topran'; 
								break;
							case 'trw automotive': 
							case 'trw/lucas': 
								$arFields["BKEY"]=						'TRW'; 
								break;
							case 'oe nissan': 
								$arFields["BKEY"]=						'Nissan'; 
								break;
							case preg_match('/nissens/', $bkey_lower) ? true : false: 
								$arFields["BKEY"]=						'Nissens'; 
								break;
							case 'oe opel': 
								$arFields["BKEY"]=						'Opel'; 
								break;
							case 'oe peugeot': 
								$arFields["BKEY"]=						'Peugeot'; 
								break;
							case 'vict_reinz': 
								$arFields["BKEY"]=						'Victor Reinz'; 
								break;
							case 'oe renault': 
								$arFields["BKEY"]=						'RENAULT'; 
								break;
							case 'oe toyota': 
								$arFields["BKEY"]=						'TOYOTA'; 
								break;
							case preg_match('/trw/', $bkey_lower) ? true : false: 
								$arFields["BKEY"]=						'TRW'; 
								break;
							case 'oe vw':
							case 'volkswagen': 
								$arFields["BKEY"]=						'VAG'; 
								break;
							case preg_match('/valeo/', $bkey_lower) ? true : false: 
							case 'phcvaleo':
							case 'phc valeo': 
						/*	case 'valeo1': 
							case 'valeo2': 
							case 'valeo wycieraczki': 
						*/
								$arFields["BKEY"]=						'VALEO'; 
								break;
							case 'calorstat by vernet': 
								$arFields["BKEY"]=						'VERNET'; 
								break;
							case 'wix filter': 
							case 'wix - filtron': 
							case 'wixfiltron': 
						...
						switch(strtolower($arFields["BKEY"])) {
							case 'fag': 
								if(preg_match('/(FAG)$/',$arFields["AKEY"])) 
									$arFields["AKEY"]= preg_replace('/(FAG$)/','',$arFields["AKEY"]);  
								break;
							case 'febi': 
								if(preg_match('/(^FE)/',$arFields["AKEY"])) 
									$arFields["AKEY"]= preg_replace('/(^FE)/','',$arFields["AKEY"]);  
								break;
							case 'topran': 
								if(preg_match('/^[0-9]{9}$/',$arFields["AKEY"])) 
									$arFields["AKEY"]= preg_replace('/[0-9]{3}$/','',$arFields["AKEY"]);  
								break;
							case 'wix':
								if(preg_match('/(WIX)$/',$arFields["AKEY"])) 
									$arFields["AKEY"]= preg_replace('/(WIX$)/','',$arFields["AKEY"]);  
								break;
						}
						$arFields["BKEY"]=strtoupper($arFields["BKEY"]);
						//INSERT / UPDATE
...
(".$qKeys.", `ORIGINAL_PRICE`) 
VALUES 
(".$qValues.", \"".$PRICE_ORIGINAL."\") 
...
".$qDuplc;
						...
						if($LnNum>=$LIMIT AND $CRON!="Y"){
							$Location = '/'.TDM_ROOT_DIR.'/admin/import/run.php?ID='.$_REQUEST['ID'].'&START='.$Line.'&INS='.$INSERTED.'&IGN='.$IGNORED.'&FP='.$arSup['FILE_PATH'].'&TEST='.$_GET['TEST'] .'&NEXT='.$_GET['nextstep'];
			...
			<?if(count($arViewFs)>0){
				...
				<br>
				<b>Демо парсинг. (!)В демо парсинге пересчет количества, цены и прочиьх полей не происходит(!).</b><br>
				<br>
