...
class TDSQL {
	...
	static public function LookupByBrandNumberCached($Brand, $Number, $TDMCore = '', $LNG_ID = TDM_LANG_ID) {
		if ($Brand == "" || $Number == "") {
			return false;
		}
		$cache=false;
		if($TDMCore!='') {
			...
			$resDB = new TDMQuery();
			$resDB->Select("TDM_TD_CACHE", array(), array("AKEY" => $Number, "BKEY" => $Brand));
			if($resDB->RowsCount>0) {
				$cache=true;
				while ($arWS = $resDB->Fetch()) {
					$local_resDB=json_decode($arWS['json'],true);
				}
			}
		}
		...
		if(!$cache) {
			...
			$resDB2 = new TDSQLQuery();
			$resDB2->QuerySelect($SQL);
			
			$resDB = $resDB2;
			$result=[];
			
			$LocalTD = new TDMQuery();
			$TDMCore->DBSelect("MODULE");
			
			while ($arWS = $resDB2->Fetch()) {
				$result[]=$arWS;
			}
			$LocalTD->Insert(
				'`TDM_TD_CACHE`', 
				array(
					'AKEY'		=> TDMSingleKey($Number),
					'BKEY'	 	=> BrandNameDecode($Brand),
